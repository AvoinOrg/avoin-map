...
const idbSet = async (key: string, value: string, config: IndexedDbStorageConfig) => {
  const db = await openIndexedDb(config)
...
const migrateFromSessionStorage = async (key: string, config: IndexedDbStorageConfig) => {
