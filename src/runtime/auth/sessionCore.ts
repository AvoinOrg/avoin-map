import type { createStartAuthCore } from './core'
import {
...
type StartAuthInstance = ReturnType<typeof createStartAuthCore>
...
type StartAuthInstanceInput = StartAuthRequestInput & {
  auth: StartAuthInstance
}
...
export const getStartAuthSessionFromAuth = async ({
  auth,
  request,
  headers,
}: StartAuthInstanceInput): Promise<StartAuthSession | null> => {
...
export const getStartAccessTokenFromAuth = async ({
  auth,
  request,
  headers,
}: StartAuthInstanceInput): Promise<StartAccessTokenResult> => {
  const requestHeaders = getHeaders({ request, headers })
  const session = await getStartAuthSessionFromAuth({
    auth,
