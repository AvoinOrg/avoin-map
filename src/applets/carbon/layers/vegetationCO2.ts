...
const getStyle = async (): Promise<ExtendedStyleSpecification> => {
  const wmtsTileUrl = buildPublicGeoServerUrl({
    path: `gwc/service/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=hiilikartta:${layerGroupId}&STYLE=&TILEMATRIXSET=EPSG:900913&TILEMATRIX=EPSG:900913:{z}&TILEROW={y}&TILECOL={x}&FORMAT=image/png`,
  })
  if (!wmtsTileUrl) {
    return {
      version: 8,
